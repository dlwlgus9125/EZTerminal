# EZTerminal 1.0.42 validation policy and residual risk

## Release identity

- Desktop, Android and native-host product version: `1.0.42`
...
1.0.42 candidate must be built from one exact clean Git SHA.
...
- upgrading from v1.0.41 repairs an unreadable installed supervisor before
  replacing it, while a fresh task install keeps child files readable;
...
- the first repair attempt creates a restricted SHA-256-verified backup before
  invoking supported non-interactive session and approval migrations;
...
or soak claim is made for 1.0.42.
