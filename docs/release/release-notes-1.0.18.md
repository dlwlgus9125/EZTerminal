...
Release identity: remote protocol v6, Android versionCode 39.

This release supersedes the unpublished 1.0.15, 1.0.16, and 1.0.17
candidates. Their immutable tags remain attached to their original commits and
were not reused.
...
- An opened menu assigns initial focus during the DOM commit, before the
  browser can deliver immediate keyboard input.
- Closing xterm search restores terminal focus in the same DOM commit, so an
  immediate paste or other terminal shortcut is not lost while waiting for an
  animation frame.

Both timing boundaries have deterministic regression coverage that remains red
when the browser delays animation frames.

## Deterministic release visuals

Responsive Storybook screenshots now wait for all three terminal panes to
finish asynchronous session binding. This prevents the release-only visual
race that could capture empty prompts before their settled working directories
appeared.
...
- Android 10 (API 29) or newer:
  `EZTerminal-Android-1.0.18-vc39.apk`
...
See the [1.0.18 validation policy](validation-policy-1.0.18.md).
