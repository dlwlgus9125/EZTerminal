...
종료·복구 경로를 상용 배포 수준으로 안정화한 패치 릴리스입니다. 1.0.3 릴리스
후보에서 확인된 Linux Android 빌드 의존성 검증 누락도 보완했습니다.
...
- Linux CI에서 내려받는 Android AAPT2 바이너리도 Google Maven의 SHA-256으로
  검증하도록 공급망 메타데이터를 보완했습니다.
- 같은 Windows 호스트에서 5회 워밍업 후 25회 측정하는 성능 비교, p95 회귀
  5% 한도, 지정 병목 15% 개선, 취소 p95 3초 이하를 RC 승인 기준으로
  적용했습니다. 게시 산출물은 동일 SHA의 보고서가 이 기준을 통과한 결과입니다.
...
- 1.0.4의 실제 프레임 캡처·OpenH264 인코딩과 `SendInput` 주입은 일반 사용자
...
Windows 빌드는 Authenticode 미서명이므로 SmartScreen 경고가 표시될 수
있습니다. Android APK는 보호된 장기키로 서명했으며, Play Store/AAB와 자동
업데이트는 제공하지 않습니다. 자세한 판정 기준은
[1.0.4 검증 정책](validation-policy-1.0.4.md)을 확인하세요.

## 다운로드

- Windows 10 22H2/Windows 11 x64: `EZTerminal-Setup.exe`
- Android 10(API 29) 이상: `EZTerminal-Android-1.0.4-vc25.apk`
