...
- 버전: 데스크톱·모바일·Android `versionName`은 `1.0.1`, 현재 패치 후보의 `versionCode`는 22
...
4. SHA를 변경하지 않은 상태에서 `v1.0.1` 태그를 push한다. workflow가 새로 검증하고 draft Release를 만든다.
...
| 파일 | 계약 |
|---|---|
| `EZTerminal-Setup.exe` | ProductVersion 1.0.1, Authenticode `NotSigned` |
| `ezterminal-1.0.1-full.nupkg` | Squirrel `RELEASES`의 SHA-1/길이와 일치 |
| `RELEASES` | 위 full package를 정확히 참조 |
| `local-rc-report.json` | exact SHA, API 29/35, 물리 Fold 승인과 30분 소크의 검증된 증거 |
| `EZTerminal-Android-1.0.1-vc22.apk` | applicationId `com.ezterminal.remote`, API 29+, 장기키 연속 서명 |
