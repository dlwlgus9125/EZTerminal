...
- [1.0.47 릴리스 노트](release-notes-1.0.47.md)
- [1.0.47 검증 정책](validation-policy-1.0.47.md)
...
이 절차는 `release/version.json`이 `full`인 경우에만 사용한다. 현재 1.0.47
...
`release-assets/1.0.47-rc-<sha8>/`에 격리한다.
...
최종 산출물은 `release-assets/1.0.47-release-<sha8>/`에만 스테이징하며,
...
4. 같은 SHA에 `v1.0.47` 태그를 push한다. 태그 workflow의 별도 publish job이
   검증된 불변 artifact를 다시 검사한 뒤 draft GitHub Release를 만든다.
5. draft의 `EZTerminal-Setup.exe`, `EZTerminal-Android-1.0.47-vc68.apk`,
...
1.0.47의 완료 지점은 검증된 draft GitHub Release다. draft를 공개하거나
...
3. 다음 산출물을 검토한다.
   ...
   | 파일 | 계약 |
   |---|---|
   | `EZTerminal-Setup.exe` | ProductVersion 1.0.47, 현재 `NotSigned`; 승인 후 `SignPath Foundation` Authenticode + timestamp |
   | `EZTerminal-Android-1.0.47-vc68.apk` | API 29+, 장기키 서명, exact build SHA |
...
6. SHA를 바꾸지 않고 `v1.0.47` 태그를 push하면 draft GitHub Release를 만든다.
