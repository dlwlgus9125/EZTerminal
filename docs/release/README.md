...
- [1.0.20 릴리스 노트](release-notes-1.0.20.md)
- [1.0.20 검증 정책](validation-policy-1.0.20.md)
...
`release-assets/1.0.20-rc-<sha8>/`에 격리한다.
...
최종 산출물은 `release-assets/1.0.20-release-<sha8>/`에만 스테이징하며,
...
3. 다음 산출물을 검토한다.
   ...
   | 파일 | 계약 |
   |---|---|
   | `EZTerminal-Setup.exe` | ProductVersion 1.0.20, Authenticode `NotSigned` |
   | `EZTerminal-Android-1.0.20-vc41.apk` | API 29+, 장기키 서명, exact build SHA |
...
6. SHA를 바꾸지 않고 `v1.0.20` 태그를 push하면 draft GitHub Release를 만든다.
