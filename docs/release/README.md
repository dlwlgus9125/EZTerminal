...
- [1.0.10 릴리스 노트](release-notes-1.0.10.md)
- [1.0.10 검증 정책과 잔여 위험](validation-policy-1.0.10.md)
...
- PC Control: 잠금되지 않은 데스크톱만 지원한다. 준비된 host service가 있을 때만 capability를 광고하며, 제어 시작에는 활성 세션 agent handshake가 추가로 필요하다. 잠금/UAC secure desktop과 Ctrl+Alt+Delete는 1.0.10에서 지원하지 않는다.
- 버전: 데스크톱·모바일·Android `versionName`은 `1.0.10`, 현재 패치 후보의 `versionCode`는 31
- 같은 SHA의 빌드 재시도는 versionCode를 올리지 않는다. 외부에 전달한 후보를 교체할 때는 32 이상으로 증가시킨다.
...
다르면 Android 키를 사용하기 전에 실패한다. 이 경로는
`release/version.json`의 `validationProfile`이 `full`일 때 유지된다.

`v1.0.10`은 모바일 대량 출력 세션의 정지를 근본 수정하고 원격 연결을
안정화하는 기능 핫픽스이며
`functional-hotfix` 프로필을 사용한다.
이 프로필은 보호된 Android 서명과 모든 공통 기능 게이트를 그대로 수행하지만,
운영자가 제외한 성능 벤치마크와 30분 소크 증거를 요구하거나 주장하지 않는다.
제한된 검증 범위와 수용 위험은
[validation-policy-1.0.10.md](validation-policy-1.0.10.md)에 고정한다.
...
2. `full` 프로필에서는 Windows 개발 PC에서 API 29/API 35 AVD를 준비하고 같은
   PC와 전원 정책에서 이전 후보의 성능 기준 보고서를 수집한 뒤 로컬 RC
   게이트를 실행한다.
...
3. `workflow_dispatch`로 Release workflow를 실행해 동일 SHA의 통합 RC 산출물을 검토한다. Playwright 릴리스 테스트는 재시도 없이 실행되고, 각 모바일 연결 시나리오는 첫 연결 결과 한 번만 허용한다. `functional-hotfix`에서는 workflow 자체의 API 29/35 및 기능 게이트가 RC 증거가 된다.
4. SHA를 변경하지 않은 상태에서 `v1.0.10` 태그를 push한다. workflow가 계약의
   검증 프로필을 다시 확인하고 draft Release를 만든다.
...
| 파일 | 계약 |
|---|---|
| `EZTerminal-Setup.exe` | ProductVersion 1.0.10, Authenticode `NotSigned` |
| `local-rc-report.json` | `full` 프로필에서만 포함되는 exact-SHA 성능·소크 증거 |
| `sbom.cdx.json` | npm·Cargo 프로덕션 의존성의 CycloneDX 1.5 SBOM |
| `EZTerminal-Android-1.0.10-vc31.apk` | applicationId `com.ezterminal.remote`, API 29+, 장기키 연속 서명 |
...
증거가 아니다. 지원 문구는 유지하되 이 미검증 범위를 버전별 검증 정책과
`release-manifest.json`에 남긴다. 잠금/UAC secure desktop과 Software SAS는
단순 미검증 범위가 아니라 1.0.10의 알려진
미지원 기능으로 별도 표시한다.
