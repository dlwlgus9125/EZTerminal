...
- [1.0.7 릴리스 노트](release-notes-1.0.7.md)
- [1.0.7 검증 정책과 잔여 위험](validation-policy-1.0.7.md)
...
- PC Control: 잠금되지 않은 데스크톱만 지원한다. 준비된 host service가 있을 때만 capability를 광고하며, 제어 시작에는 활성 세션 agent handshake가 추가로 필요하다. 잠금/UAC secure desktop과 Ctrl+Alt+Delete는 1.0.7에서 지원하지 않는다.
- 버전: 데스크톱·모바일·Android `versionName`은 `1.0.7`, 현재 패치 후보의 `versionCode`는 28
- 같은 SHA의 빌드 재시도는 versionCode를 올리지 않는다. 외부에 전달한 후보를 교체할 때는 29 이상으로 증가시킨다.
...
`v1.0.7`은 PC Control 기능 핫픽스이며 `functional-hotfix` 프로필을 사용한다.
이 프로필은 보호된 Android 서명과 모든 공통 기능 게이트를 그대로 수행하지만,
운영자가 제외한 성능 벤치마크와 30분 소크 증거를 요구하거나 주장하지 않는다.
제한된 검증 범위와 수용 위험은
[validation-policy-1.0.7.md](validation-policy-1.0.7.md)에 고정한다.
...
4. SHA를 변경하지 않은 상태에서 `v1.0.7` 태그를 push한다. workflow가 계약의
...
| 파일 | 계약 |
|---|---|
| `EZTerminal-Setup.exe` | ProductVersion 1.0.7, Authenticode `NotSigned` |
| `local-rc-report.json` | `full` 프로필에서만 포함되는 exact-SHA 성능·소크 증거 |
| `sbom.cdx.json` | npm·Cargo 프로덕션 의존성의 CycloneDX 1.5 SBOM |
| `EZTerminal-Android-1.0.7-vc28.apk` | applicationId `com.ezterminal.remote`, API 29+, 장기키 연속 서명 |
