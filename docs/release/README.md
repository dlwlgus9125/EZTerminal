...
- [1.0.35 릴리스 노트](release-notes-1.0.35.md)
- [1.0.35 검증 정책](validation-policy-1.0.35.md)
...
이 절차는 `release/version.json`이 `full`인 경우에만 사용한다. 현재 1.0.35
...
`release-assets/1.0.35-rc-<sha8>/`에 격리한다.
...
최종 산출물은 `release-assets/1.0.35-release-<sha8>/`에만 스테이징하며,
...
4. 같은 SHA에 `v1.0.35` 태그를 push한다. 태그 workflow의 별도 publish job이
   검증된 불변 artifact를 다시 검사한 뒤 draft GitHub Release를 만든다.
5. draft의 `EZTerminal-Setup.exe`, `EZTerminal-Android-1.0.35-vc56.apk`,
...
3. 다음 산출물을 검토한다.
   ...
   | 파일 | 계약 |
   |---|---|
   | `EZTerminal-Setup.exe` | ProductVersion 1.0.35, 현재 `NotSigned`; 승인 후 `SignPath Foundation` Authenticode + timestamp |
   | `EZTerminal-Android-1.0.35-vc56.apk` | API 29+, 장기키 서명, exact build SHA |
...
6. SHA를 바꾸지 않고 `v1.0.35` 태그를 push하면 draft GitHub Release를 만든다.
