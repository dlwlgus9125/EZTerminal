...
- 동일한 깨끗한 Git SHA에서 버전 계약, 타입 검사, 린트, 루트·모바일 단위
  테스트와 일반 데스크톱 E2E를 실행합니다.
- Windows 설치 파일과 Android 프로덕션 APK를 같은 SHA에서 빌드하고,
  Android APK는 기존 장기 키로 서명한 뒤 버전·인증서 지문을 확인합니다.
- Android 10(API 29)에서 모바일 시작 PTY를 실행한 채 앱을 강제 종료하고,
  프로세스 재생성 후 출력 재생과 입력 제어 복구를 실제로 확인합니다.
- 운영자의 명시적 요청에 따라 `pnpm e2e:performance`,
...
- 이번 SHA에 대한 데스크톱 성능 회귀 비교와 30분 모바일 소크는 수행하지
  않았으므로 이 릴리스는 그 두 항목의 새 검증 증거를 주장하지 않습니다.
