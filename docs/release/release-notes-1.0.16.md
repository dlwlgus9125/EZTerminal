...
Release identity: remote protocol v6, Android versionCode 37.

This release supersedes the unpublished 1.0.15 candidate. The immutable
v1.0.15 tag remains attached to its original commit and was not reused.
...
## Reliable keyboard menus

An opened menu now assigns initial focus during the DOM commit, before the
browser can deliver immediate keyboard input. This prevents fast Enter or
arrow-key input from racing the trigger button and also makes the Storybook
interaction gate deterministic on release runners.
...
- Android 10 (API 29) or newer:
  `EZTerminal-Android-1.0.16-vc37.apk`
...
See the [1.0.16 validation policy](validation-policy-1.0.16.md).
