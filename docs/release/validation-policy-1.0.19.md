# EZTerminal 1.0.19 validation policy and residual risk

## Release identity

- Desktop, Android, and native-host product version: `1.0.19`
...
The tagged 1.0.14 through 1.0.18 identities and their documentation are
...
- GitHub metadata parsing rejects drafts, prereleases, non-stable tags,
  duplicate or incompatible assets, unbounded files, missing digests, and
  non-official release URLs.
- Desktop tests cover coalesced checks, manifest verification, verified
  downloads, unsigned acknowledgement, integrity mismatch cleanup,
  cancellation, and unavailable Downloads paths.
- Android JVM tests cover the exact official asset URL, redirect allowlist,
  and release size limits. Android lint and both API-level instrumentation
  lanes compile and exercise the registered native plugin.
- Mobile tests cover version/versionCode monotonicity, rate limits, native
  download handoff, progress state, and installer permission state.
- Ordinary Electron E2E runs with live update checks disabled so mutable
  GitHub state cannot make unrelated release validation nondeterministic.
...
Release 1.0.19 therefore uses the `functional-hotfix` profile and does not
...
- The functional gates do not install the release through a physical Android
  device's unknown-app permission screen or download the full published setup
  asset from GitHub after publication. Package identity, signer, digest, and
  installer-intent boundaries are covered below that physical-device seam.
