...
| ✅ **테마/외형** | 색상·폰트·투명도·배경 | **완료(2026-07-02)** — dark/light/high-contrast 빌트인, settings.json 영속, xterm 라이브 재테마. **+ Matrix(CRT) 테마 추가(2026-07-04, 커밋 ba213a7): 4번째 빌트인, self-host woff2(Share Tech Mono/VT323 — 패키지 CSP `font-src 'self'` 대응), `[data-theme='matrix']` 스코프 CRT 효과.** **+ 확장형 테마 mod·토글 이펙트 카탈로그·번들 폰트 피커(v0.6.0, 2026-07-08): 검증 JSON 테마 mod(Zod `ThemeModSchema`), `getActiveTheme()`가 크롬(CSS vars)·xterm 양쪽 인지, 데스크톱+모바일 공유 렌더러.** **+ CRT 롤바 이펙트(v0.6.1~v0.6.6): 움직이는 roll → 사용자조절(count/두께/spread/색/속도/opacity/softness) → 심리스 컨베이어 → wide/faint 기본값.** **+ CRT 간섭 4종(v0.6.7): burst/micro jitter·static noise·flicker.** 투명도·커스텀 색상은 후속 |
...
| ✅ **시스템 상태 패널** | CPU/MEM/NET/DISK/PROC 실시간 모니터 + 패킷 캡처 | **완료(2026-07-04)** — 우측 300px 비모달 오버레이 드로어. CPU 코어 그리드 + MEM 상세(swap) + NET 스파크 + 연결 목록 + DISK/PROC(`systeminformation` 1Hz + 개방시 PowerShell 수집기). **실시간 패킷 캡처**(`cap`+Npcap, off-by-default, 1회 승인 게이트, 헤더 전용, 전용 `src/packet-capture/` utilityProcess·MessagePort 직접 브로커). |
...
| ✅ **모바일 원격 제어** | 데스크톱 셸을 안드로이드 앱에서 원격 제어 | **완료(2026-07-05)** — `EzTerminalApi`+MessagePort seam을 WS로 재구현(`src/main/remote-bridge.ts`, 토큰 인증)해 렌더러 `BlockController`/블록 컴포넌트를 **무수정 재사용**. Capacitor 안드로이드 앱(`mobile/`, `ws-ezterminal.ts` 트랜스포트 + auth 워치독 재연결) + 데스크톱 페어링 패널(M4) + 에뮬레이터 자동 e2e 스모크. stats는 범위 제외. 설계: `docs/design/mobile-remote-control-design.md`. |
...
- ✅ **상태 오버레이 패널 완료 (2026-07-04):** 우측 300px 비모달 드로어 — CPU/MEM 순수 JS 상시(60s 스파크) + 개방 시 NET/PROC/DISK PowerShell 수집기. `src/main/system-stats-service.ts` · `src/renderer/StatusPanel.tsx` · IPC `stats:update`/`stats:history`/`stats:panel-visible`. 베이스라인: vitest 361 · e2e 66 · audit 0.
- ✅ **상태 패널 v2 완료 (2026-07-04):** CPU 코어 그리드(`currentLoad.cpus`, `cpus ?? []` 가드) + MEM 상세(available/cached/swap) + NET rx/tx 스파크 + 연결 목록(`networkConnections`) + **실시간 패킷 캡처**(native `cap` + Npcap, off-by-default, 1회 승인 게이트, 헤더 전용) — 전용 `src/packet-capture/` utilityProcess(`vite.packet-capture.config.ts` 6번째 빌드 엔트리, `MessageChannelMain` 직접 브로커). deps `systeminformation@5.31.11`(정확 핀)·`cap@^0.2.1`, 가드 `guard:native-cap`, `.npmrc clang=false`·`rebuildConfig.ignoreModules` 빌드 픽스. 베이스라인: **vitest 372 · e2e 73 · audit 0**.
- ✅ **Matrix(CRT) 테마 완료 (2026-07-04, 커밋 ba213a7 → origin/main):** 4번째 빌트인 — 녹색 인광 팔레트, self-host woff2, `[data-theme='matrix']` 스코프 CRT 효과(스캔라인·글로우). tsc 0 · vitest 372 · e2e 73 · audit 0.
- ✅ **모바일 원격 제어 완료 (2026-07-05, team 실행 — Sonnet 워커 + 리드 재현검증):** 데스크톱을 띄워두면 안드로이드 앱에서 stats 제외 모든 셸 세션을 제어(라이브 출력·키 입력·세션 전환·생성/종료·스크롤백). **핵심: `EzTerminalApi`(`window.ezterminal`)+per-command MessagePort seam을 WebSocket으로 재구현 → `BlockController`·블록 컴포넌트 무수정 재사용.** M0 브리지(`src/main/remote-bridge.ts` DI seam·electron 미import·토큰 인증 4001·per-run MessageChannelMain relay·pty-data base64, `session-directory`·`remote-token-store`) → M1 트랜스포트 심(`mobile/src/transport/ws-ezterminal.ts`, FakeMessagePort+dispatchEvent) → M2 Capacitor 웹셸(`mobile/`) → M2b Android debug APK → M3 에뮬레이터 자동 e2e(`mobile/e2e/smoke.ts`, adb+uiautomator, Appium 불필요) → M4 데스크톱 페어링 패널. **리드 독립검증이 M3의 무효 명령(`echo`는 이 셸 명령 아님→`cmd /c echo hello`) 오류를 실측(Node WS 클라이언트)으로 적발·수정.** 실기기(Z Fold7)+Tailscale로 `ls` 구조화 테이블 라이브 왕복 확인. **재연결 견고성 수정**: App의 연결 타임아웃이 트랜스포트를 죽이던 버그 → 트랜스포트 auth 워치독(half-open 자가치유)+타임아웃이 죽이지 않고 자동 재연결. 실측 함정: ws 번들 크래시(vite external+packageAfterPrune)·Capacitor androidScheme http·usesCleartextTraffic·tslib hoisted. 설계·함정 전체: `docs/design/mobile-remote-control-design.md`. 베이스라인: **root vitest 418·e2e 73·audit --prod 0 · mobile vitest 23**.
- 🟢 **GitHub 원격 · CI 가동 (2026-07-04):** 저장소 생성·푸시 완료 — `github.com/dlwlgus9125/EZTerminal`(**private**), `main` 푸시됨. GitHub Actions CI 가동 중(`windows-2022` 러너로 핀 — `windows-latest`의 VS18을 `@electron/node-gyp`가 못 읽어 `cap` 빌드 실패하던 것 해소). 릴리스 태그(v0.1.0)·자동업데이트(B-M4)는 여전히 결정 대기(공개 여부·서명 등).
- ✅ **v0.2.0 설정 UI + 전체 UI 스케일 + 원격 브리지 토글 + 탭 오버플로 (2026-07-06):** 데스크톱 우측 설정 드로어(⚙️ — 테마 라디오 4종·UI 스케일 80–150%·원격 온오프·버전; stats·pairing과 3자 배타) + 모바일 동일 뷰. rem 기반 루트 스케일(터미널+크롬 동반 확대/축소, xterm 리핏), `remoteEnabled`(끄면 WS 리스너 미기동), dockview 탭 스트립 오버플로/휠 스크롤에 4테마 스타일. **v0.3.0 (2026-07-06): 설정 드로어의 중복 테마 섹션 제거**(테마 전환은 헤더 순환 버튼·모바일 🎨 ThemeMenu만).
- ✅ **v0.4.0 미러 레벨-트리거 attach + v0.5.0 컨트롤 핸드오프 (2026-07-06):** 데스크톱에서 이미 실행 중인 run(claude 같은 TUI 포함)에 모바일/보조 페인이 뒤늦게 붙어도 라이브 표시 — run 발견이 1회 브로드캐스트뿐이던 것을 `list-runs` 조회 attach로 해소 + `pty-resize` 프라이머리 전용 게이트 + `pty-dims` 미러 사이징 + 미러 xterm 자동응답(DA/DSR) 억제. **v0.5.0:** 미러 "Take control" 버튼으로 리사이즈 권한 이전(공유 PTY가 그 기기 크기로 재배치, 컨트롤 기기 끊기면 자동 복귀) + 미러 shrink-to-fit(가로 스크롤 대신 글자 자동 축소). **v0.5.1 (2026-07-07): 화면 클릭 시 cmd-input 포커스 복구 + 제출 시 입력창 클리어(데스크톱).**
- ✅ **v0.6.0 확장형 테마 mod + 토글 이펙트 + 폰트 피커 (2026-07-08):** 하드코딩 4테마 → 확장·모딩 시스템(검증 JSON `ThemeModSchema` — id charset·`CSS.escape`·cssVars 화이트리스트, `getActiveTheme()` 리졸버가 크롬 CSS vars·xterm 양쪽 인지) + 이름 붙은 이펙트 카탈로그(`html[data-effect-*]`, 이펙트별 토글) + 번들 폰트 피커(self-host woff2 Share Tech Mono/JetBrains Mono/Fira Code + Cascadia). 데스크톱 Electron + Capacitor 안드로이드 공유 렌더러.
- ✅ **v0.6.1~v0.6.7 CRT 이펙트 시리즈 (2026-07-08):** crt-rollbar + scanline-scroll(움직이는 roll) → 사용자조절 5제어(count/두께/gap/색/속도) → spread%·균일 줄 → opacity + gradient softness → 심리스 컨베이어(line count 제거, 두께 200px) → wide/faint Matrix-green 기본값 → **CRT 간섭 4종(burst/micro jitter·static noise·flicker 정상화)**. 전부 독립 토글 + 전용 슬라이더, 데스크톱·모바일 공통, 기본 OFF(Matrix가 선언). 검증=음수 animation-delay 위상 고정 픽셀 추출 + paused 상태 data-effect 재토글로 애니메이션 재생성. **현재 베이스라인(2026-07-08, v0.6.7): unit(root vitest) 654 · mobile vitest 104 · e2e 108 · packaged 8 · audit --prod 0.**
- ✅ **OpenClaw 관리 기능 완료 (2026-07-12, team 실행 — deep-interview 확정 + Wave 0~4, feat/openclaw-management 17커밋, main 미머지):** 로컬 OpenClaw 게이트웨이(예약작업 상시구동, `127.0.0.1:18789`)를 EZTerminal 안에서 상태·수명주기·세션·로그·설정 + **채팅**(Control UI 임베드)으로 관리 — 데스크톱·모바일 전체 패리티. **수명주기는 관찰+제어 위임뿐**(EZTerminal 종료가 게이트웨이에 무영향, `openclaw gateway start/stop/restart` CLI 위임). 데스크톱 채팅은 iframe/webview가 아니라 메인 소유 `WebContentsView`(Control UI가 `X-Frame-Options: DENY`로 임베드 자체 차단하기 때문 — 전용 파티션, 토큰은 메인이 `#token=` 프래그먼트로 조립해 렌더러엔 절대 미전달). 모바일 채팅은 EZTerminal 자체 역프록시(`openclaw-proxy.ts`, 포트 7421, `remoteEnabled`와 동일 수명)로 3중 방어(티켓+소스IP 바인딩 인증, Origin 재작성, frame-ancestors 재작성) 돌파. **실 게이트웨이로 AC2(데스크톱 채팅) 완전 검증**(ping→pong GPT-5.5 실 왕복). **AC5(모바일 채팅)는 인프라 전부 검증되고 최종 벽 1개만 남음**: `http://<lan-ip>` 접속이 브라우저 insecure context라 Gateway의 device-identity 생성이 막히는 문제(데스크톱은 `127.0.0.1`이 secure context라 무료 통과) — 해법은 게이트웨이 호스트에서 `openclaw config set gateway.controlUi.allowInsecureAuth true` + 재시작(코드로 우회 불가, 사용자 결정 — 계획은 EZTerminal이 사용자의 OpenClaw 설치를 대신 바꾸지 않는다는 원칙으로 HTTPS/Tailscale Serve 대안을 의도적으로 기각). M6 스윕에서 사용자 승인으로 이 설정이 실제 적용됨(라이브 게이트웨이 재시작, 신 PID). 상세 설계·함정 전체: `docs/design/openclaw-management-design.md`. 검증: tsc/lint 0 · **root vitest 865 · mobile vitest 147 · e2e 124(123 그린 + 무관 pre-existing 플레이크 1건, 원인 격리로 확정 — `status-panel.spec.ts:47`, OpenClaw 헤더 버튼 유무와 무관하게 동일 재현) · packaged smoke 8/8 · guard:native/native-cap/pty-routing 전부 그린**. 시각 검증(드로어·채팅패널·모바일 탭·AC2 왕복) 전부 스크린샷 직접 육안 확인. main 병합·푸시·모바일 실기기 최종 검증은 사용자 결정.
- ⬜ **잔여:** B-M4 자동업데이트(공개 저장소·서명 결정 대기) → E3 AI 보조(인터뷰+게이트, **데이터 이그레스 동의 대기**) → E6 크로스플랫폼(mac/linux 실검증). 사용자 결정은 `docs/NEXT-SESSION.md` 참조.
