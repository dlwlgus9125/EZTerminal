...
| ✅ **테마/외형** | 색상·폰트·투명도·배경 | **완료(2026-07-02)** — dark/light/high-contrast 빌트인, settings.json 영속, xterm 라이브 재테마. **+ Matrix(CRT) 테마 추가(2026-07-04, 커밋 ba213a7): 4번째 빌트인, self-host woff2(Share Tech Mono/VT323 — 패키지 CSP `font-src 'self'` 대응), `[data-theme='matrix']` 스코프 CRT 효과.** 투명도·커스텀 색상은 후속 |
...
- ⬜ **잔여:** B-M4 자동업데이트(공개 저장소·서명 결정 대기) → E3 AI 보조(인터뷰+게이트, **데이터 이그레스 동의 대기**) → E6 크로스플랫폼. 사용자 결정은 `docs/NEXT-SESSION.md` 참조.
