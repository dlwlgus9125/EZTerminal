...
if (!pnpmCli) {
  throw new Error('e2e:performance must be run through pnpm');
}
...
if (dirty) {
  throw new Error(
    'release performance measurement requires a clean worktree:\n'
      + dirty,
  );
}

const reportPath = process.env.EZTERMINAL_PERFORMANCE_REPORT_PATH
  || path.join(root, 'release-assets', 'desktop-performance-report.json');
const result = spawnSync(
  ...
  ],
  {
    ...
    env: {
      ...
      EZTERMINAL_PERFORMANCE_REPORT_PATH: reportPath,
    },
    stdio: 'inherit',
    windowsHide: true,
  },
